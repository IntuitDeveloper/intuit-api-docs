...
<<<<<<< HEAD
Intuit Payroll is engaged  in partnerships with 401(k) plan providers to offer 401(k) benefit to Small Business Owners. This will allow businesses to provide retirement benefits to their employees. This guide will help you understand the high level integration.
=======
...
>>>>>>> 5549b00165b1c8c8b15dee25d0d041e7db14d779
...
* Implement SSO using oAuth2.0 authentication implementation which confirms to OpenID Connection speficification
...
401(k) Providers will have their own screens for sign up and enrollment of employees of the company, but will require that information be transferred back to QBO Payroll so that the correct deductions can be taken. 
 
They will then need to write (and read) these fields for contribution per employee level. The only person who can edit the names or fields of each contribution will be the creator. If you are taking over from a different company as 401(K) provider you will need to work with QBO company's payroll admins to sunset contributions from previous plan. 
